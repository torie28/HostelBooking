    ...
    const getFilteredStudents = () => {
        ...
        if (studentGenderFilter) {
            filtered = filtered.filter(student => {
                const hostel = hostels.find(h => h.name === student.hostel);
                return hostel && hostel.gender === studentGenderFilter;
            });
        }

        // Filter by hostel
        if (studentHostelFilter) {
            filtered = filtered.filter(student => student.hostel === studentHostelFilter);
        }
    ...
    const getFilteredRooms = () => {
        ...
        if (roomGenderFilter) {
            filtered = filtered.filter(room => {
                ...
                return hostel && hostel.gender === roomGenderFilter;
            });
        }

        // Filter by hostel
        if (roomHostelFilter) {
            filtered = filtered.filter(room => room.hostel_id == roomHostelFilter);
        }
                    ...
                    <div className={`bg-white shadow rounded-lg transition-all duration-500 ease-in-out transform ${isTransitioning ? 'opacity-0 translate-x-4 scale-95' : 'opacity-100 translate-x-0 scale-100'
                        ...
                        <div className="px-4 py-5 sm:p-6">
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 mb-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setStudentHostelFilter(e.target.value)}
                                            className="w-full px-3 py-2 border border-gray-300 rounded-full shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                        >
                                            <option value="">All Hostels</option>
                                            {hostels.map((hostel) => (
                                ...
                                {/* Reset Filters Button */}
                                {(studentGenderFilter || studentHostelFilter) && (
                                    <div className="mt-4">
                                        <button
                                            onClick={() => {
                                                setStudentGenderFilter('');
                                                setStudentHostelFilter('');
                                            }}
                                            className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-full text-gray-700 bg-black/10 focus:outline-none"
                                        >
                                            Reset Filters
                                ...
                                <div className="mt-4 text-sm text-gray-600">
                                    ...
                                    {(studentGenderFilter || studentHostelFilter) && (
                                        <span>
                                            {studentGenderFilter && <span className="ml-2">â¢ Gender: {genders.find(g => g.value === studentGenderFilter)?.label}</span>}
                                            {studentHostelFilter && <span className="ml-2">â¢ Hostel: {studentHostelFilter}</span>}
                                        </span>
                                    )}
                                </div>
                            </div>
